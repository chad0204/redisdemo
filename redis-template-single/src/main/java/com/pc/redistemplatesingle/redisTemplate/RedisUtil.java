package com.pc.redistemplatesingle.redisTemplate;
...
public class RedisUtil {
    ...
    public boolean lock(String lockName, long timeout) {
        ...
        String expireAt = String.valueOf(currentTime + timeout);
        if (!redisTemplate.getConnectionFactory().getClusterConnection().setNX(lockName.getBytes(), expireAt.getBytes())) {
            String getExpireAt = redisTemplate.opsForValue().get(lockName);
            if (getExpireAt != null) {
                if (currentTime >= Long.parseLong(getExpireAt)) {
                    byte[] getSetExpireAt = redisTemplate.getConnectionFactory().getClusterConnection().getSet(lockName.getBytes(), expireAt.getBytes());
                    if (getSetExpireAt != null) {
                        getLock = new String(getSetExpireAt).equals(getExpireAt);
                    }
                } else {
                    getLock = false;
                }
